#!/usr/bin/env bash
# Run set_fiscal_position_exento.py: create fiscal position "Exento de impuestos" with Detectar de forma automática.
# Run after accounting (and ideally l10n) is installed. From repo root:
#   sudo -E bash install/scripts/run_set_fiscal_position_exento.sh
# Or: sudo ODOO_VERSION=19 bash install/scripts/run_set_fiscal_position_exento.sh
set -euo pipefail
...
SET_FP_SCRIPT="${SCRIPT_DIR}/set_fiscal_position_exento.py"
...
RUN_SCRIPT="/tmp/set_fiscal_position_exento_odoo.py"
...
echo "Done. Fiscal position 'Exento de impuestos' with Detectar de forma automática is set."
