#!/usr/bin/env bash
# Run set_default_products_pa.py: create service products (Servicio de Acarreo, Otros Gastos, Seguro) with 0% tax.
# Run after sale/product and set_default_taxes_pa. From repo root:
#   sudo -E bash install/scripts/run_set_default_products_pa.sh
set -euo pipefail
...
SET_SCRIPT="${SCRIPT_DIR}/set_default_products_pa.py"
...
RUN_SCRIPT="/tmp/set_default_products_pa_odoo.py"
...
echo "Done. Default service products (0% tax) are available."
