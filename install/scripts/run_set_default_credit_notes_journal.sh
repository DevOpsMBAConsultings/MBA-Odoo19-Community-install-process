#!/usr/bin/env bash
# Run set_default_credit_notes_journal.py: create or find journal "Notas de crédito" and set as default for credit notes.
# Run after accounting (and ideally l10n) is installed. From repo root:
#   sudo -E bash install/scripts/run_set_default_credit_notes_journal.sh
# Or: sudo ODOO_VERSION=19 bash install/scripts/run_set_default_credit_notes_journal.sh
set -euo pipefail
...
SET_JOURNAL_SCRIPT="${SCRIPT_DIR}/set_default_credit_notes_journal.py"
...
RUN_SCRIPT="/tmp/set_default_credit_notes_journal_odoo.py"
...
echo "Done. New customer credit notes (notas de crédito) will use the default journal."
