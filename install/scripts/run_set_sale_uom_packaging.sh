#!/usr/bin/env bash
# Run set_sale_uom_packaging.py: enable Units of measure and packaging in Sales.
# Run after sale (and preferably product/stock) are installed. From repo root:
#   sudo -E bash install/scripts/run_set_sale_uom_packaging.sh
set -euo pipefail
...
SET_SCRIPT="${SCRIPT_DIR}/set_sale_uom_packaging.py"
...
RUN_SCRIPT="/tmp/set_sale_uom_packaging_odoo.py"
...
echo "Done. Unidades de medida y embalajes enabled in Sales."
